locals{
  table = local.ignore_read_write_changes ? aws_dynamodb_table.this_autoscaling : aws_dynamodb_table.this 
}
...
resource "aws_appautoscaling_target" "table_read" {
  ...
  resource_id        = "table/${local.table[0].name}"
...
resource "aws_appautoscaling_target" "index_read" {
  ...
  resource_id        = "table/${local.table[0].name}/index/${each.key}"
